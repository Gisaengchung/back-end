...
module.exports = class Project {
  ...
  static async insert({ 
    ...
    projectLocState,
    projectLocCity,
    projectMainImage = 'https://res.cloudinary.com/gisaengchung/image/upload/v1615399664/default/dwwpfykc2q73twhdfaho.png',
    projectFundingGoal,
    projectFundingExDate,
    projectRiskChallenge,
    projectDiversity,
  ...
  static async increment({
    ...
    const { rows } = await pool.query(
      `UPDATE projects 
      SET 
      donations = CAST($2 AS DECIMAL) + CAST(donations AS DECIMAL)
  ...
  static async findDonationInfo(projectId) {
    ...
    const { rows } = await pool.query(
      `SELECT  
        ...
        donations,
        funding_goal,
        CONCAT(substring(funding_ex_date, 6, 2),'/',substring(funding_ex_date, 9, 2),'/',substring(funding_ex_date, 0, 5)) AS funding_ex_date,
        risk_challenge,
        diversity,
        user_id
      FROM projects
